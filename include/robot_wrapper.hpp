...
public:
  RobotWrapper(
    ...
    cmd_vel_sub_ = node_->create_subscription<geometry_msgs::msg::Twist>(
        ...
        std::bind(&RobotWrapper::cmd_vel_callback, this,
                  std::placeholders::_1));
    ...
    if (mod->GetModelType() == "position") {
      ...
      if (mod->GetModelType() == "ranger") {
        ...
        rangers_.push_back(std::make_shared<RangerWrapper>(
            node_, static_cast<Stg::ModelRanger *>(mod), model_name,
            tf_prefix_));
      } else if (mod->GetModelType() == "camera") {
        mod->Subscribe();
        cameras_.push_back(std::make_shared<CameraWrapper>(
            node_, static_cast<Stg::ModelCamera *>(mod), model_name,
            tf_prefix_));
      } else {
        RCLCPP_WARN_STREAM(node_->get_logger(), "sensor "
                                                    << mod->GetModelType()
                                                    << "is not supported");
      }
    }
