...
public:
  CameraWrapper(
    const rclcpp::Node::SharedPtr & node, Stg::ModelCamera * model, const std::string & name,
  ...
  : model_(model)
  {
    ...
    image_pub_ =
      node->create_publisher<sensor_msgs::msg::Image>(std::string("~/") + name + "/image_raw", 10);
    depth_pub_ =
      node->create_publisher<sensor_msgs::msg::Image>(std::string("~/") + name + "/depth", 10);
    // TODO(pokusew): verify that get_parameter_or really works
    node->get_parameter_or("is_depth_canonical", is_depth_canonical_, true);
  }

  void publish(
    const std::shared_ptr<tf2_ros::TransformBroadcaster> & tf_broadcaster, const rclcpp::Time & now)
  {
    publish_image(now);
    publish_depth(now);
