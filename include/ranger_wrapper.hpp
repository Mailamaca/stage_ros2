...
public:
  RangerWrapper(
    const rclcpp::Node::SharedPtr & node, Stg::ModelRanger * model, const std::string & name,
  ...
  : model_(model)
  {
    ...
    laser_scan_pub_ =
      node->create_publisher<sensor_msgs::msg::LaserScan>(std::string("~/") + name, 10);
    ...
    const std::shared_ptr<tf2_ros::TransformBroadcaster> & tf_broadcaster, const rclcpp::Time & now)
  {
    ...
    msg.angle_increment =
      static_cast<float>(sensor.fov / static_cast<double>(sensor.sample_count - 1));
