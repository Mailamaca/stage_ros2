...
void CameraWrapper::publish_depth(const rclcpp::Time &now) {
  ...
  if (is_depth_canonical_) {
    ...
    auto depth_min =
        static_cast<uint16_t>(model_->getCamera().nearClip() * 1000.f);
    auto depth_max =
        static_cast<uint16_t>(model_->getCamera().farClip() * 1000.f);
...
void CameraWrapper::publich_camera_info(const rclcpp::Time &now) {
