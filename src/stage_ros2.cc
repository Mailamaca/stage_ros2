...
public:
  void init(int argc, char **argv) {
    ...
    world_->Start();
    ros2_thread_ =
        std::make_shared<std::thread>([this]() { executor_->spin(); });
  }
...
private:
  ...
  int search_and_init_robot(Stg::Model *mod) {
    RCLCPP_DEBUG_STREAM(node_->get_logger(),
                        "[search robots] token: " << mod->Token() << ", type: "
                                                  << mod->GetModelType());
    if (mod->GetModelType() == "position") {
      ...
      mod->ForEachDescendant(
          [](Stg::Model *mod, void *user) {
            return static_cast<StageWrapper *>(user)->search_and_init_sensor(
                mod);
          },
          this);
    }
  ...
  int search_and_init_sensor(Stg::Model *mod) {
    RCLCPP_DEBUG_STREAM(node_->get_logger(),
                        "[search sensors] token: " << mod->Token() << ", type: "
                                                   << mod->GetModelType());
